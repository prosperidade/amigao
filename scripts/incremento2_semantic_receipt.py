...
def receipt(tenant_id=4):
    with SessionLocal() as db:
        ...
        obs = [r for r in latest.values() if r.kind == 'observacao']
        result = {'tenant': tenant_id, 'documents': [], 'jobs': [], 'checks': {}}
        for doc in docs:
            rows = [r for r in obs if r.source_document_id == doc.id]
            rejected = latest.get(f'extracao:rejeicoes:{doc.id}')
            failures = rejected.content['attributes']['normalized']['rejeicoes'] if rejected else []
            anchored = 0
            for row in rows:
        ...
        result['checks']['receita_death_observation'] = bool(items(557, 'falecimento_declarado'))
