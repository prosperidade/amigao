...
- Afirmação que não passa na verificação **não é emitida**; a geração falha com a lista dos IDs
  ...
  (dívida #283): ela poderá reescrever `texto`, nunca `evidencias`.
...
- **Redator por LLM já neste incremento.** A exigência é evidência por ID em cada afirmação; a
  ...
  afirmações fixas (dívida #283), depois de o validador existir — que é o que este ADR entrega.
...
**Custos e riscos.**
- A tela não conhece relatório, escopo nem orçamento; o percurso é por API (dívida **#282**,
  irmã da #278).
- A redação é esquemática, não prosa de consultor (dívida **#283**, com o checklist de TR da
  peça definitiva).
- O legado `OrcamentoAgent._estimate_by_rules` e a `PRICE_TABLE` continuam no código para o
  caminho sem orçamento; saem quando a #282 fechar (dívida **#284**).
