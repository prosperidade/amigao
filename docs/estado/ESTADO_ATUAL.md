...
**Pulso 2026-06-03 (CI — Backend Lint verde):** o check `backend-lint` estava vermelho desde #52 — diagnóstico provou **(A) infra + (B) código**: o CI instalava só `requirements.txt` mas ruff/mypy vivem em `requirements-dev.txt` → `ruff: command not found` (exit 127), o lint **nunca rodou**; atrás disso, 77 erros reais de ruff e ~495 de mypy. Fix: `ci.yml` instala `requirements-dev.txt` (lint+test), ruff zerado (62 autofix + 15 manuais sem mudar comportamento; 2 `# noqa` justificados — B027 hook opcional, B017 teste de WS), mypy vira **advisory** (`continue-on-error`, dívida #46), ruff/mypy **pinados**. Validado: ruff 0; suíte 753/753 (baseline e pós-fix idênticos). RUNBOOK_DEV ganhou seção de lint. Doc: `docs/trabalhos/backend_lint.md`. Branch `fix/backend-lint`.
