...
**Pulso 2026-06-30 (ROTA REGULATÓRIA E5 — snapshot editável — `feat/rota-e5-sprint2`, ADR-021):** as `etapas` da `LegislacaoAgent` eram efêmeras (só no JSON do `AIJob`, sem tabela/tela, perdidas ao recarregar — medido no diagnóstico read-only). Este sprint materializa a Rota como **entidade editável e assinável**. Entidades `Rota`+`RotaPasso` (migration `d1e2f3a4b5c6`, `dedupe_key`+UNIQUE parcial desde o commit 1 — **4º caso** a nascer certo, dívida #48). `rota_materializer.py` reconstrói o `Etapa` **TIPADO** (`sources`+`prazo_fonte`) do dual-emit — nunca o bruto top-level (que quebra o schema strict); reconciliação **aditiva/não-destrutiva** (ADR-017): preserva edição/ordem/classificação/manual; rota validada + diff → `desatualizada`. Endpoints gerar/reordenar/editar/manual/validar/**fechar**: validar exige classificação (Ficha §8.1), fechar exige todos validados e grava `AuditLog` com **hash chain SHA-256** — 1º uso real da cadeia (dívida #18). UI: na E5 a aba **Ações vira a Rota** (não é 7ª aba), drag via `framer-motion <Reorder>` (sem `@dnd-kit`), badge de fonte honesto, toggle faturável/direção. **Decisões travadas (André):** demand_type-driven (não religar a chain agora), ler o típado, @dnd-kit rejeitado, "nenhum passo sem norma" na validação, dedupe é higiene. **Validação:** 18 testes novos verdes + `tsc` limpo + migration round-trip no vereda_dev. **Follow-ons #49–#53** (doc em Saídas, religar auditor→legislacao, gatilho ação→rota, auto-RAG de fundamento, aprendizado das reordenações). Doc `docs/adr/021-rota-e5-snapshot-editavel.md`.

**Pulso 2026-06-30 (verificação espacial DERIVADA, não armazenada — `fix/derivar-verificacao-espacial-d1`, ADR-020):** decisão travada do André após diagnóstico read-only do caso 13. Os 11 `VERIFICACAO_ESPACIAL_PENDENTE` **sobreviveram a tudo** porque (a) o wipe nunca tocou `regulatory_issues` e (b) o cleanup retroativo **nunca rodou em prod**. Causa de fundo: um placeholder informativo (`geom IS NULL` → "análise espacial não rodou") era **armazenado** como achado e re-emitido a cada E2/E4. **Fix (modelo "derivar, não armazenar", Princípio 11):** removido o ramo `geom is None` em `property_audit.py` (auditor para de emitir); nova nota **derivada na leitura** via `GET /properties/{id}/diagnosis-notes` (`source="derived"`, `acionavel=false`) quando `geom IS NULL`; UI `DiagnosisTab` separa **achados** (AlertaCard completo) de **notas** (linha discreta, sem botões); código aposentado no catálogo. Quando D1 popular geom, a seção emite achados ESPACIAIS REAIS (persistidos). **Validação:** 201 backend (anti-regressão: audit_property/auditor NÃO emitem; endpoint deriva) + 76 frontend, tsc/eslint verdes. **Ordem obrigatória:** código → deploy → confirmar 0 emissão → SÓ ENTÃO limpar prod (DELETE total do tipo aposentado, dry-run aprovado antes). Constraint UNIQUE (3º caso dedupe-sem-constraint) → dívida #48. Doc `docs/adr/020-verificacao-espacial-derivada.md`.

**Pulso 2026-06-30 (alertas regulatórios duplicados 11× — `fix/alertas-regulatorios-duplicados`):** medido no caso 13 (property 10, Supabase prod, read-only): o mesmo `VERIFICACAO_ESPACIAL_PENDENTE` tinha **11 linhas idênticas** (ids 18–28), `detected_at` espalhado por ~2 semanas → **GERAÇÃO** (não render): `AuditorImovelAgent._persist_issues` inseria 1 issue por finding **a cada** re-execução da chain, sem dedupe. Fix: guard de idempotência em `_persist_issues` (reusa issue não resolvida com mesma `issue_dedupe_key` = `property+codigo_alerta+tema+descricao`, preservando `status_achado`) + `app/services/regulatory_dedupe.py` + `scripts/sanear_alertas_duplicados.py` (idempotente, mirror de `sanear_staging.py`; preserva decisões; **reporta** conflitos sem apagar). Escopo global: só 1 grupo afetado. Caso 13: o script remove as 9 `suspeita`, preserva 22 (confirmada) e 23 (descartada) → **pendência ao André**: 2 decisões conflitantes sobre o mesmo achado (resetar p/ 1 ou escolher qual). **Validação:** `test_regulatory_dedupe.py` (9) + regressão auditor/regulatory/diagnostico verdes (55). Saneamento prod + aceite são pós-deploy. Doc `docs/trabalhos/alertas_duplicados.md`. NÃO tocou máquina de transição/gates (território do Sprint 1).

**Pulso 2026-06-29 (SPRINT 1 — ramo condicional E2→E3/E4 — `feat/sprint1-ramo-e2`, ADR-019):** a saída da E2 (Diagnóstico Preliminar) deixou de ser linear e virou **ramo** (Ficha 07): `MACROETAPA_TRANSITIONS[diagnostico_preliminar]` agora tem **dois destinos** (`coleta_documental`, `diagnostico_tecnico`); `resolve_next_macroetapa(current, has_essential_pending)` recomenda Coleta (E3) se há documento essencial pendente (`ProcessChecklist` required+pending = `missing_docs`) ou pula direto ao Diagnóstico Técnico (E4) se não há. Doc pendente **roteia** (não trava) só na E2 (`list_macroetapa_blockers` ganhou `current_macroetapa`); a E4 é alcançável direto da E2 (gate da E4 é condição, não "E3 anterior"); a E3 pulada vira `skipped` no stepper (badge não mente). Avanço segue confirmado pelo consultor (ADR-018) — o ramo só decide o destino. TASK 1 dispensada (sinal canônico já existia; `DiagnosticoAgent` não tocado). **Validação:** `test_ramo_e2.py` prova os 2 caminhos reais + audit; bateria transição/gate 97 verdes; regressão processes+regulatory 100 verdes; tsc/build limpos. Doc `docs/trabalhos/sprint1_ramo_e2.md`.

**Pulso 2026-06-29 (SPRINT 0 — aba "Conferência" — `feat/sprint0-aba-conferencia`):** decisão Isis+André (Ficha 07): a aba **"Alertas" virou "Conferência"** (label em `ProcessDetailTypes`, mantendo `key:'alertas'`). `AlertasTab`→**`ConferenciaTab`** renderiza só o `ConsolidacaoPanel` (campos a conferir + divergências) + empty state; o botão **"Gravar na base"** (ex-"Consolidar na base") virou **rodapé fixo (sticky)** sempre à vista (lógica de consolidação intacta). Os **alertas regulatórios** do auditor saíram da Conferência e nascem na **Visão geral** (`DiagnosisTab`, abaixo da assinatura), cada um com botão **"→ Ações"** que cria uma Ação via `useCreateAcao` (fluxo existente; alerta não vira ação sozinho). `onGoToAlerta` agora só rola (mesma aba). **Validação:** 73 testes vitest (novos: "→ Ações" cria ação, label "Conferência"), tsc/eslint/build verdes. Doc `docs/trabalhos/sprint0_conferencia.md`. NÃO refez limpeza/histórico (#84/#85/#86). Follow-ups: vincular alerta→ação com `issue_id`/origem auditor (precisa endpoint); botão "Gravar" pode migrar de seção (Sprint próprio).

**Pulso 2026-06-29 (limpeza RETROATIVA do staging + tradução de termos — `fix/staging-limpeza-retroativa-e-traducao`):** o Hub do caso 13 já enche (item zero fechado no #81), mas a tela **"Decisão & Consolidação"** seguia suja: o #81 limpou a extração para casos NOVOS e a limpeza-na-origem **não retroage** — o staging do caso 13 entrou antes. **TASK 1 (saneamento retroativo):** novo `sanear_staging_process` (`ficha01_extraction.py`) **reusa os helpers do #81** (`_dedup_token`/`_numeric_dedup_key`/`_is_garbage_for_code`/`_CODE_FIELDS`/`_LIST_COLLAPSE_FIELDS`) e aplica ao staging gravado: (2a) duplicata de formato (`349.9022`≡`349,9022`), (2b) lixo em campo de código (preservando decisão do consultor), (2c) lista repetida (`pendencias_rat`/`onus`) colapsada por `(fonte, campo, hint, token)`. Valores realmente diferentes e fontes distintas **não** se fundem; a canônica preserva a decisão; **idempotente**. Comando `scripts/sanear_staging.py --process-id 13 [--dry-run]` (padrão `reindex_sync`, no RUNBOOK). **TASK 2 (tradução):** `regulatory_issues` (target_field do `pendencias_rat`) caía no fallback → **"Regulatory issues"** cru; adicionado ao `fieldLabels.ts` como **"Pendências regulatórias"** (+`pendencias_rat`/`*_declarada_ha`). `ConsolidacaoPanel` já passava tudo por `labelFor`/`STATUS_LABEL` — o único furo era o dicionário. **Validação:** `test_staging_saneamento_retroativo.py` (10) + `test_ficha01_staging_limpeza.py` (7, #81 intacto), suíte completa verde, `tsc` verde, sem migration. Doc: `docs/trabalhos/staging_limpeza_retroativa.md`. Branch `fix/staging-limpeza-retroativa-e-traducao`.

**Pulso 2026-06-29 (consolidação dava 500 + limpeza do staging — `fix/consolidacao-clique-grava`):** depois do #79, o André clicou "Consolidar na base" no caso 13 e **nada gravava** (`audit consolidar`=0, `field_sources={}`, 0 matrículas). Medido no console: `POST /consolidar → 500`. **Causa-raiz reproduzida localmente:** `psycopg2 can't adapt type 'dict'` — o extrator stage `averbacao_app`/`averbacao_rl` como **dict estruturado** e `consolidate_process` gravava o dict numa coluna **Text** → 500 → **rollback de tudo** (por isso nem audit). O botão/endpoint/gate estavam certos; quebrava no commit. **Fix (TASK 1):** `_coerce` serializa dict/list em texto legível (`_stringify_structured`) quando a coluna é `String`/`Text` (JSON portável preserva o dict). **Limpeza do staging na origem (TASK 2, `ficha01_extraction.py`):** (2a) dedup de **formato** — "349.9022"≡"349,9022" via `_numeric_dedup_key` (último separador=decimal); (2b) **lixo em campo de código** — "Certidão de Embargo"/"Coordenadas não disponíveis"/"PRAD" descartados em `_CODE_FIELDS` (reusa `_is_doc_title` do #72); (2c) **campo-lista** (`pendencias_rat`/`onus`) colapsa em 1 por (campo, matrícula); dedup vale intra-doc e **cross-run**. **Validação:** `test_repro_caso13.py` (consolidação grava: matrícula/property/audit/3 ações) + `test_ficha01_staging_limpeza.py` (7) + regressão fase4/acoes/matriz/matricula (29) + ficha01/validators/extrator (28), ruff limpo, sem migration. Pós-deploy: 1 clique enche o Hub. Doc: `docs/trabalhos/consolidacao_clique_e_staging.md`. Branch `fix/consolidacao-clique-grava`.

**Pulso 2026-06-28 (FIX da consolidação — gate destravado + parcial + ponte RL — `fix/consolidacao-gate-divergente`):** o **item zero do MVP**. A consolidação do staging na base real **nunca executou** em prod (`audit_logs action='consolidar'`=0; 0 matrículas) — causa medida (não re-investigada): `ConsolidacaoPanel.tsx` desabilitava o botão com `pendentesObrig > 0`, então **UM** `divergente_transcricao` prendia os 27 campos já aceitos. O endpoint `/consolidar` e o serviço `consolidate_process` estavam corretos e plugados, só nunca foram invocados. **Decisão Isis (opção b):** consolidação **PARCIAL** — os consistentes gravam e cada divergente não resolvido vira uma **`Acao`** (`origem=consolidacao`, novo valor de enum via migration `c8d4e1a2f9b0`; com fonte #70; idempotente por `dedupe_key`; `divergente_fundo` segue como achado da matriz, não vira ação). **Gate destravado:** `disabled` agora só por `consolidaveis === 0`; UI diz "N gravam · M viram ações". **Ponte matrícula→imóvel (Princípio 11):** `rl_status` entrou na allowlist do imóvel (antes `rl_declarada_ha→imovel.rl_status` caía em `ignorados` → Hub "—"); se o imóvel não tem RL e ≥1 matrícula tem `averbacao_rl`, deriva `rl_status='averbada'` com `field_sources['rl_status']='derived_matricula'` (transparente); **APP não** é parseada de texto livre — `app_area_ha` só de dado estruturado nível-imóvel ("—" proposital). **Audit:** `action='consolidar'` passou a registrar (com hash chain — Princípio 2) quem/quando/campos/ações, disparando também quando só há ação. **Validação:** `test_fase4_consolidacao.py` **9 verdes** (+3: parcial, ponte RL, allowlist RL nível-imóvel), `test_acoes`+`test_property_hub_derivacao` 11 verdes, `tsc`/`build` verdes; migration provada em Postgres real (`downgrade -1`→`upgrade head`). Aceite no process 13 real é pós-deploy (manual). Doc: `docs/trabalhos/fix_consolidacao.md` + ADR-017 + FLUXOS_E2E (Fluxo 8). Branch `fix/consolidacao-gate-divergente`.
...
- 37+ telas/abas em 10 áreas (Auth, Clients, Processes, Properties, Intake, Contracts, Proposals, Dashboard, AI, Settings)
  ...
  - **Movimentação do card (Fase 0.2, 2026-06-29):** o card anda pelas 7 macroetapas. Intake cria os 7 `MacroetapaChecklist` (+ backfill lazy p/ legado); botão **"Rodar agentes da etapa"** (`POST /macroetapa/run-agents`) dispara a chain da etapa → ao concluir, o worker marca o checklist (`mark_stage_agents_done`) → card `pronta_para_avancar` → consultor confirma em "Avançar etapa". De-inversão ADR-018 (avançar não dispara mais chain). Ramo E2→E3|E4 e gates finos E5..E7 = fase seguinte. Ver `docs/trabalhos/movimentacao_card.md`.
