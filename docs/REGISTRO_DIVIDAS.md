...
> **PRÓXIMO NÚMERO LIVRE: 280.** (#273 a #278 reservadas pela frente 4b — motor jurídico, PR #208;
> #279 aberta pela frente #271/#272, 23/09.)
...
| **222** | Cobertura declarada mentia quando a última fatia falhava no LLM (achado Codex 10/09) | 2026-09-10 (`fix/cobertura-janela`) | `_extract_structured` gravava `cobertura_chars = fatias[-1].fim` — a última fatia PLANEJADA, não a processada. Se ela falhasse no gateway ou no parse do JSON, a janela declarava cobertura completa (`truncado=False`) sem ter lido o trecho. **Fix:** `cobertura_chars = por_fatia[-1][0].fim` (última fatia processada **com sucesso**); `JanelaResultado` ganhou `completa: bool` e `falhas: list[dict]` (índice/intervalo/erro de cada fatia que falhou — LLM, exceção inesperada ou JSON inválido); `resumo()` expõe `completa`/`falhas` quando há falha, sem tocar no shape quando não há. `truncado` mantém o significado antigo (cobertura ficou abaixo do total, por qualquer motivo); `completa` é o sinal específico de "alguma fatia planejada não foi lida". Testes em `tests/services/test_extraction_window.py`: falha simulada na última fatia de um doc de 2 → cobertura = fim da fatia 0, `completa=False`, falha listada com índice/intervalo/erro; controle com as 2 fatias OK → cobertura = fim do doc, `completa=True`, `falhas=[]`. |
