# Registro de dívidas — Regente (consolidado pós-PROMPT_10 · 2026-05-26)
...
| **23** | Gate camada 2 cobrando decisão em achado terminal (trap revelado pós-PROMPT_9) | 2026-05-26 (PROMPT_10) | Gate de `PATCH /diagnoses/{version}/validate` agora filtra `status_achado in {suspeita, confirmada}` — `descartada`/`resolvida`/`ignorada` não pedem decisão (o consultor já adjudicou que não há divergência ativa a tratar; exigir seria dupla negação). `suspeita` permanece pra forçar adjudicação antes de assinar (não é deadlock — UI/endpoint permitem mover o estado). `resolved_at IS NULL` mantido como critério ortogonal (estado do campo persistido). 4 testes novos no `TestValidateDiagnosisGateCamada2` cobrem cada terminal liberando o gate + um teste explícito que `confirmada` sem decisão continua 422. Sem migration, sem ADR (refina camada 2 já firmada). **Follow-on aberto:** badge "N pendentes" do `DiagnosisAssinatura` (PROMPT_9) precisa espelhar a mesma exclusão pra não super-contar — não esta rodada. |
