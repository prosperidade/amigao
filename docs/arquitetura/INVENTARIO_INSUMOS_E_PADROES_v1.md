...
| INS-004 | `MANUAIS_SEMAD.rar` localizado; correspondência com o pacote integral de 224 itens ainda não verificada | Contagens/itens/duplicatas continuam pendentes da próxima inspeção. |
...
`99!E4` afirma que alerta não impede avanço, conclusão, protocolo ou status. O plano v1.1 e o padrão 8 exigem suspensão do **uso de claim/regra afetado por conflito material**, preservando a decisão fundamentada humana. Essas duas coisas não são equivalentes: uma autorização de avançar o fluxo não publica uma conclusão sem lastro. Além disso, `98!F19` exige revisão jurídica em efeito material e a cartorária conserva comandos de bloqueio. **PENDENTE-ISIS:** definir precisamente ação suspensa, autoridade da exceção e registro da decisão; não resolver esta tensão reescrevendo o original ou liberando consumo automático. No escopo de execução, a referência continua sendo a v1.1.
...
O diretório também contém CAR v05; não foi promovida a versão vigente nem substituiu o ZIP v03 expressamente solicitado. **PENDENTE-ISIS:** qual pacote/hash é a edição publicável, qual a linhagem do exemplo e qual o destino dos IDs ausentes? O nome com `(1)` da fundiária não decide versão: os 28 IDs existem também na v03, mas o controle 99 declara mudanças de comportamento.
...
| Q-ISIS-03 — PENDENTE-ISIS | Para cada linha aprovada, quais pares matriz/versão/ID de regra são necessários e suficientes, com vínculo explícito a um ou mais objetivos? Qual tratamento deseja para objetivo não identificado ou misto? | Não há coluna objetivo nas oito abas v01; os agrupadores Tema/Eixo estão contados por UF, mas não definem o objetivo canônico. Há colisões de IDs entre matrizes. |
...
| Q-ISIS-08 — PENDENTE-ISIS | Qual arquivo/hash é a edição publicável da matriz 2 e da CAR? O ZIP v03 contém um único exemplo CAR, com REG-BR-CAR-001 reutilizado para outro conteúdo e 36 IDs ausentes: é recorte, complemento ou substituição, e qual a linhagem/destino de cada ID? A CAR v05 encontrada deve entrar numa comparação posterior? | A contagem histórica de três não se reproduz. Bloqueia publicação/reconciliação; ausência não revoga regras. |
| Q-ISIS-09 — PENDENTE-ISIS | Qual arquivo/hash/aba/célula contém a condição sob “Tipo de regra”? Nas cópias fornecidas só há rótulos em CAR E5:E41/v03 E5; M02 E15 é condição conforme E4. Houve outra edição ou leitura por posição? Qual dado deve ser corrigido, preservando o literal? | Defeito relatado não reproduzido; não corrigir dados por hipótese nem importar por posições uniformes. |
| Q-ISIS-10 — PENDENTE-ISIS | Nas 174 regras cartorárias, quais conjuntos são suficientes por objetivo/UF para título versus registro, completude da certidão, transmissão, sucessão, representação, baixa e aditivo? Confirma e completa as correspondências com as 325 linhas v01, evitando alertas duplicados? | Matriz aberta e contada; suficiência e deduplicação semântica não decorrem de ID ou quantidade. |
| Q-ISIS-11 — PENDENTE-ISIS | Como compatibilizar 99!E4, que permite avançar/concluir/protocolar apesar de alerta, com 98!F19, os bloqueios cartorários e a suspensão do uso de claim/regra com conflito material exigida na v1.1? Qual ação fica suspensa, quem pode decidir e que fundamentação/registro libera o uso? | Tabela integral transcrita; tensão de governança explícita. Não liberar conclusão sem lastro nem escolher vencedor por esfera/ranking. Incremento 4. |
...
| Q-ISIS-17 — PENDENTE-ISIS/ANDRÉ | Dentro do escopo futuro confirmado de agentes para advogados ambientais, quais papéis podem propor, revisar juridicamente, aceitar e publicar cada tipo de peça, e quais objetivos/UF/ritos serão homologados primeiro? Quem valida a aplicabilidade/efeito de um precedente? | A decisão de incluir esse consumidor está tomada; permissões e cobertura específicas ainda não. Orienta o schema compartilhado sem implementar agora. |
...
| B — Cobertura | 16 demandas + Jobson; consumidor jurídico futuro confirmado e R2 como pendência de infraestrutura, sem implementação. Seleção de lançamento pendente. |
