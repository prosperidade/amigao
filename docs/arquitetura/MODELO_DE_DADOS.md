...
| `ExtractedFieldStaging` | `extracted_field_staging` | **Ficha 01 / FASE 1 (ADR-015).** Staging de campos extraídos: agentes propõem (extrator/auditor), consultor decide, sistema grava na base. Carrega `confidence`, `status` (enum `extractedfieldstatus`: pendente/consistente/divergente_transcricao/divergente_fundo/aceito/rejeitado), `target_entity`/`target_field`, `matricula_hint`, e rastreabilidade (`created_by_agent`, `ai_job_id`). FK `process_id` (CASCADE), `document_id`/`ai_job_id`/`decided_by_user_id` (SET NULL). Nada escreve aqui na FASE 1 (extrator passa a escrever na fase 2). **`tipo_observacao`/`atributos` (ADR-065):** O QUE um ato de matrícula É (`String(40)` indexada, vocabulário fechado em `app/services/observacao_registral.py`: reserva_legal, app, hipoteca, alienacao_fiduciaria, arrendamento, baixa, …) e os atributos que o tipo pede (`JSONB`: ato, data, área, valor, partes, prazo, ato_referenciado). Nulos em campo de cabeçalho — só ato registral é observação; destino é decidido DEPOIS do tipo, por um mapa enumerado (`DESTINO_POR_TIPO`), e tipo sem coluna correspondente fica visível sem destino em vez de forçado numa gaveta alheia. |
