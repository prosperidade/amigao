...
class TestNonBlockingReviewAgents:
    ...
    def test_chain_para_quando_agente_BLOCKING_pede_review(self):
        """Regressão: agentes fora de NON_BLOCKING (ex.: diagnostico) param a chain."""
        ctx = _ctx()
        executed: list[str] = []
        ...
        # Chain parou em legislacao; diagnostico NÃO rodou
        assert executed == ["extrator", "auditor_imovel", "legislacao"]
        assert len(results) == 3
        assert "diagnostico" not in executed
