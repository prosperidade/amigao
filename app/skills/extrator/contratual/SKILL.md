...
Contrato com ESPÓLIO não pode ser descartado por não preencher cadastro. Preserve a parte espólio, o vínculo nominal ao falecido e a pessoa indicada como inventariante; o número do inventário vai em `referencias_processo[]` com `natureza` inventario e `sujeito` no espólio. O espólio não recebe CPF, CNPJ nem OAB: `identificador` e `tipo_identificador` da parte espólio ficam nulos. O CPF escrito junto ao espólio é do falecido: extraia o falecido como parte `pf` própria, com esse CPF no trecho dela, e aponte `falecido_chave` do espólio para essa parte. Preserve OAB como identificador profissional próprio, nunca como CPF nem como prova de nomeação. A menção ao inventariante é participação declarada mesmo sem termo de nomeação: registre as lacunas de documento, alcance e validade. Não complete essas lacunas pela data do contrato. A escritura anexada ao inventário continua escritura; sua origem não a transforma em certidão nem prova sucessão concluída.
