...
const STATUS_CONFIG: Record<string, { label: string; dotCls: string; textCls: string }> = {
  backlog:       { label: 'Pendente',     dotCls: 'bg-slate-500',   textCls: 'text-slate-400' },
  a_fazer:       { label: 'A Fazer',      dotCls: 'bg-blue-400',    textCls: 'text-blue-400' },
  em_progresso:  { label: 'Em Progresso', dotCls: 'bg-yellow-400 animate-pulse', textCls: 'text-yellow-400' },
  aguardando:    { label: 'Aguardando',   dotCls: 'bg-orange-400',  textCls: 'text-orange-400' },
  revisao:       { label: 'Revisão',      dotCls: 'bg-purple-400',  textCls: 'text-purple-400' },
  concluida:     { label: 'Concluída',    dotCls: 'bg-emerald-400', textCls: 'text-emerald-400' },
  cancelada:     { label: 'Cancelada',    dotCls: 'bg-red-400',     textCls: 'text-red-400' },
};
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-white/5 border border-white/10 p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Trilha Regulatória</p>
            <p className="text-base font-semibold text-white">
              {ws.template_name ?? (ws.demand_type ? `Tipo: ${ws.demand_type}` : 'Sem trilha definida')}
            </p>
            {ws.demand_type && (
              <p className="text-xs text-slate-400 mt-0.5">Tipo de demanda: {ws.demand_type}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-2xl font-bold text-white">{ws.completion_pct}%</p>
              <p className="text-xs text-slate-500">{ws.completed_steps}/{ws.total_steps} etapas</p>
            </div>
      ...
      {/* Lista de etapas */}
      {allSteps.length === 0 ? (
        <div className="rounded-2xl bg-white/5 border border-dashed border-white/10 p-10 text-center">
        ...
        <div className="relative pl-6 border-l border-white/10 space-y-0">
          {allSteps.map((step: any, idx: number) => {
            ...
            const isPending = !step.task_status || step.task_status === 'backlog';
            ...
            return (
              <div key={step.order} className={`relative pb-6 ${isLast ? 'pb-0' : ''}`}>
                {/* Dot */}
                <div className="absolute -left-[31px] bg-slate-950 p-0.5">
                  {isDone
