...
try {
  for (const origin of ['23', '25']) {
    ...
    const obs = [...latest.values()].filter(r => r.object.kind === 'observacao');
    ...
    if (origin === '23') {
      ...
      checks.transmitente_visible = await page.getByText(/declaração extraída — transmitente/).first().isVisible();
